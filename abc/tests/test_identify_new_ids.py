...
2. Are flase added ids foud and correctly annotated 
"""
...
file_path = '/tests_directory/iic/identify_new_ids.py'
...
print alglist('identify new ids')
result = runalg(
    "script:identifynewids",
    input_layer,
    updated_layer,
    None
)
...
class TestOriginalIDCheck(unittest.TestCase):
    ...
    def test_false_add(self):
        # test for false additions
        found = False
        request = QgsFeatureRequest().setFilterExpression('t50_fid = 123')
        for feature in output_layer.getFeatures(request):
            self.assertEqual(feature['t50_origid'], 123)
            self.assertEqual(feature['error_type'], 'false added id')
            found = True
        self.assertEqual(found, True)

        found = False
        request = QgsFeatureRequest().setFilterExpression('t50_fid = 32050671')
        for feature in output_layer.getFeatures(request):
            self.assertEqual(feature['t50_origid'], 32050671)
            self.assertEqual(feature['error_type'], 'false added id')
            found = True
        self.assertEqual(found, True)
